...
For cross-model searching, this plugin keeps track of every model that acts_as_indexed.
...
Use the +IndexedModels.with_query+ method to search every model that acts_as_indexed.
You can specify which models to include or exclude using +:only+ and +:except+.

  IndexedModels.with_query 'michael jackson', :only => [:article, :review]

The results are returned grouped by model.

  [[#<Article id:1>, #<Article id:2>], [#<Review id: 1>]]
...
* ^"":: A quoted term that begins with ^ matches any phrase that begins with this phrase. '^"cat d"' will find records matching the whole phrases "cat dog" and "cat dinner". When a ^"" term is used, all other terms are ignored. This type of search is useful for autocomplete inputs.
