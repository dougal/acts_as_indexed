...
It is recommended that you turn thread safety off when running on Heroku,
see the 'Further Configuration' section above, and the relevant section in
<tt>lib/acts_as_indexed/configuration.rb</tt> for more information. When a
non-writable root directory is detected, thread safety will turn off by default.
