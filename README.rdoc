...
To search, call the +with_index+ named scope on your model, passing a query as 
the first argument.

  # Returns array of Post objects.
  my_search_results = Post.with_index('my search query')
  
  # Chain it with any number of ActiveRecord methods and named_scopes.
  my_search_results = Post.public.with_index('my search query').find(:all, :limit => 10) # return the first 10 matches which are public.
...
Since +with_index+ is a named scope, WillPaginate can be used in the normal fashion.

  @images = Image.with_index('girl').paginate(:page => 1, :per_page => 5)

== RDoc Documentation

To generate the RDoc documentation, run the <tt>rake rdoc</tt> task in the acts_as_indexed 
plugin folder. Then point your web browser at <tt>vendor/plugins/acts_as_indexed/rdoc/index.html</tt>.
...
* UTF-8 support. See the current solution here: https://gist.github.com/193903bb4e0d6e5debe1
