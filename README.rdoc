...
==== By default the following query operators are supported (default_operator set to :and)

* AND :: This is the default option. 'cat dog' will find records matching 'cat' AND 'dog'.
* AND :: You may also be specific here. '+cat +dog' 
...
* 'cat dog -eagle -falcon' will find records with 'cat' AND 'dog' but without both 'eagle' and 'falcon'. So records having 'cat', 'dog' and 'eagle' are matched!

==== When specifying the default_operator to :or

You also have the opportunity to set the default operator to OR by passing an option like this (or in the call to +acts_as_indexed+):

    ActsAsIndexed.configure do |config|
      config.default_operator = :or
      # More config as required...
    end

Thus the operators are:

* OR  :: This is the default option. 'cat dog' will find records matching 'cat' OR 'dog', with records matching both having a better relevance.
* AND :: '+cat +dog' will find records that contains both 'cat' AND 'dog'
* NOT :: 'cat -dog' will find records matching 'cat' AND NOT 'dog'
* INCLUDE :: 'cat +me' will find records matching 'me', even if 'me' is smaller than the +min_word_size+, and the relevance will be higher if those records also contains 'cat'
* "" :: Quoted terms are matched as phrases. '"cat dog"' will find records matching the whole phrase. Quoted terms can be preceded by the NOT operator; 'cat -"big dog"' etc. Quoted terms can be preceded by the AND operator; 'cat +"big dog" will find records with 'big dog' and possibly 'cat'. Quoted terms can include words shorter than the +min_word_size+.
* ^ :: Terms that begin with ^ will match records that contain a word starting with the term. '^cat' will find matches containing 'cat', 'catapult', 'caterpillar' etc.
* ^"" :: A quoted term that begins with ^ matches any phrase that begin with this phrase. '^"cat d"' will find records matching the whole phrases "cat dog" and "cat dinner". This type of search is useful for autocomplete inputs.

Examples of more advanced queries:

* 'cat dog -eagle -falcon' will find records with 'cat' OR 'dog' but without any of 'eagle' or 'falcon'. So records having 'cat', 'dog' and 'eagle' are NOT matched!
* 'cat +dog -eagle -falcon' will find records with 'dog' that may containt 'cat' but without any of 'eagle' or 'falcon'. Matches records: 'dog' 'dog cat', does not match records: 'cat eagle' 'dog falcon'
