...
This branch has super-basic support for stemming.  It's probably
better than nothing, but it has the major flaw that it doesn't respect
quoting.  So, if you search for "fruits fly" (with quotes), you could
get matches for "fruit flies".  Also, the partial-word match is
probably ever-so-slightly broken.  What we really need is a separate
exact-word index.  I'm contemplating it.
